var sectorShape; function Stage3Create() {
  // キーボード有効
  cursors = this.input.keyboard.createCursorKeys();
  // 全ての音を止める
  this.sound.stopAll();
  // BGMを再生
  const Stage3BGM = this.sound.add('Stage3');
  Stage3BGM.play({
    volume: BGMvolume,// volumeの種類設定
    loop: true        // ループ設定
  });
  Stage3BGM.setVolume(BGMvolume);// 音量を設定
  // 背景アニメーションの設定
  ...
  platforms.create(0, 200, 'ground').setScale(1).refreshBody();    // 地面の描画
  platforms.create(800, 200, 'ground').setScale(1).refreshBody();    // 地面の描画
  platforms.create(400, 315, 'ground').setScale(0.5,1).refreshBody();    // 地面の描画
  platforms.create(0, 430, 'ground').setScale(1).refreshBody();    // 地面の描画
  platforms.create(800, 430, 'ground').setScale(1).refreshBody();    // 地面の描画
  platforms.create(400, 630, 'ground').setScale(2).refreshBody();    // 地面の描画
  ...
  warp = this.physics.add.sprite(400, 545, "Warp");
  ...
  const text = this.add.text(width / 2 - 80, height / 2 - 60, "Stage １－３", {
    font: "24px Arial",
    fill: "#ffffff" // 白色の文字
  });
  ...
  this.physics.add.overlap(player, warp, hitwarp3, null, this);
