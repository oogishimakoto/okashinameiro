var sectorShape; function Boss2Create() {
  // キーボード有効
  cursors = this.input.keyboard.createCursorKeys();
  // 全ての音を止める
  this.sound.stopAll();
  // BGMを再生
  const Boss1BGM = this.sound.add('Boss1');
  Boss1BGM.play({
    volume: BGMvolume,// volumeの種類設定
    loop: true        // ループ設定
  });
  Boss1BGM.setVolume(BGMvolume);// 音量を設定
  ...
  player = this.physics.add.sprite(enemyCoordinateX, enemyCoordinateY, "player");
  ...
  warp = this.physics.add.sprite(750, 545, "Warp");
  ...
  const text = this.add.text(width / 2 - 55, height / 2 - 60, "ジョニー", {
    font: "24px Arial",
    fill: "#ffffff" // 白色の文字
  });
  ...
  // 敵同士当たり判定をつける
  this.physics.add.collider(enemy_johnny, platforms);
  ...
  this.physics.add.overlap(player, warp, hitwarpBoss, null, this);
  this.physics.add.overlap(player, yami, hitenemy, null, this);
  this.physics.add.overlap(player, sickle, hitenemy, null, this);
  this.physics.add.overlap(player, enemy_johnny, hitenemy, null, this);
  this.physics.add.overlap(player, enemy_specter, hitenemy, null, this);
  ...
  // ライトと星が当たったら
  this.physics.add.overlap(hitchek, enemy_johnny, collectEnemy, null, this);
  this.physics.add.overlap(hitchek, enemy_specter, collectEnemy, null, this);
