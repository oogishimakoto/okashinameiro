var sectorShape; function Stage4Create() {
  // キーボード有効
  cursors = this.input.keyboard.createCursorKeys();
  // 全ての音を止める
  this.sound.stopAll();
  // BGMを再生
  const Stage4BGM = this.sound.add('Stage4');
  Stage4BGM.play({
    volume: BGMvolume,// volumeの種類設定
    loop: true        // ループ設定
  });
  Stage4BGM.setVolume(BGMvolume);// 音量を設定
  // 背景アニメーションの設定
  ...
  platforms.create(100, 585, 'ground').setScale(0.3,1).refreshBody();    // 地面の描画
  platforms.create(200, 555, 'ground').setScale(0.3,1).refreshBody();    // 地面の描画
  platforms.create(300, 525, 'ground').setScale(0.3,1).refreshBody();    // 地面の描画
  platforms.create(400, 495, 'ground').setScale(0.3,1).refreshBody();    // 地面の描画
  platforms.create(400, 495, 'ground').setScale(0.3,1).refreshBody();    // 地面の描画
  platforms.create(500, 525, 'ground').setScale(0.3,1).refreshBody();    // 地面の描画
  platforms.create(600, 555, 'ground').setScale(0.3,1).refreshBody();    // 地面の描画
  platforms.create(700, 585, 'ground').setScale(0.3,1).refreshBody();    // 地面の描画
  ...
  player = this.physics.add.sprite(0, 800, "player");
  ...
  warp = this.physics.add.sprite(400, 425, "Warp");
  ...
  const text = this.add.text(width / 2 - 80, height / 2 - 60, "Stage １－４", {
    font: "24px Arial",
    fill: "#ffffff" // 白色の文字
  });
  ...
  this.physics.add.overlap(player, warp, hitwarp4, null, this);
