var sectorShape; function Stage1Create() {
  // キーボード有効
  ...
  // BGMを再生
  const Stage1BGM = this.sound.add('Stage1');
  Stage1BGM.play({
  ...
  platforms.create(10, 230, 'ground').setScale(1).refreshBody();    // 地面の描画
  platforms.create(800, 400, 'ground').setScale(1).refreshBody();    // 地面の描画
  ...
  warp = this.physics.add.sprite(770, 330, "Warp");
  ...
  const text = this.add.text(width / 2 - 80, height / 2 - 60, "Stage １－１", {
    font: "24px Arial",
    fill: "#ffffff" // 白色の文字
  });
  ...
  this.physics.add.overlap(player, warp, hitwarp1, null, this);
