var sectorShape; function Stage5Create() {
  // キーボード有効
  cursors = this.input.keyboard.createCursorKeys();
  // 全ての音を止める
  this.sound.stopAll();
  // BGMを再生
  const Stage5BGM = this.sound.add('Stage5');
  Stage5BGM.play({
    volume: BGMvolume,// volumeの種類設定
    loop: true        // ループ設定
  });
  Stage5BGM.setVolume(BGMvolume);// 音量を設定
  ...
  const text = this.add.text(width / 2 - 80, height / 2 - 60, "Stage １－５", {
    font: "24px Arial",
    fill: "#ffffff" // 白色の文字
  });
  ...
  this.physics.add.overlap(player, warp, hitwarp5, null, this);
